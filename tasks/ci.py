...
def pylint(ctx, loc="local", tests=False, everything=False, specific=""):
    """
    pylint machine-learning-with-python folder
    Usage: inv ci.pylint
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    if tests:
        ...
        ctx.run(
            "pylint --output-format=colorized --rcfile ./lint-configs-python/python/pylintrc tests machine_learning_with_python"
        )
    elif specific:
        ctx.run(
            f"pylint --output-format=colorized --disable=all --enable={specific} --rcfile ./lint-configs-python/python/pylintrc tests machine_learning_with_python"
        )
    else:
        ctx.run(
            "pylint --output-format=colorized --disable=all --enable=F,E --rcfile ./lint-configs-python/python/pylintrc machine_learning_with_python"
        )
...
def mypy(ctx, loc="local", verbose=0):
    """
    mypy machine-learning-with-python folder
    Usage: inv ci.mypy
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    # ctx.run("mypy --config-file ./lint-configs-python/python/mypy.ini machine_learning_with_python tests")
    ctx.run("mypy --config-file ./setup.cfg machine_learning_with_python tests")
...
def black(ctx, loc="local", check=False, debug=False, verbose=0, tests=False):
    ...
    _cmd += "machine_learning_with_python"
...
def isort(ctx, loc="local", check=False, dry_run=False, verbose=0, diff=False):
    """
    isort machine-learning-with-python module. Some of the arguments were taken from the starlette contrib scripts. Tries to align w/ black to prevent having to reformat multiple times.
    ...
    _cmd += " machine_learning_with_python tests"
...
def pre_start(ctx, loc="local", check=True, debug=False):
    """
    pre_start machine-learning-with-python module
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    # ctx.run("python machine_learning_with_python/api/tests_pre_start.py")
...
def pytest(
    ...
    _cmd += r" --cov-config=setup.cfg --verbose --cov-append --cov-report=term-missing --cov-report=xml:cov.xml --cov-report=html:htmlcov --cov-report=annotate:cov_annotate  --showlocals --tb=short --cov=machine_learning_with_python tests"
