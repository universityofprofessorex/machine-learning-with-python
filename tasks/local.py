...
#     # ctx.run("python ./machine_learning_with_python/api/backend_pre_start.py")
#     # ctx.run("python ./machine_learning_with_python/initial_data.py")
#     # ctx.run("python machine_learning_with_python/dev_serve.py")
...
def rsync(ctx, loc="local", verbose=0, cleanup=False):
    """
    rsync over files to ~vagrant/machine-learning-with-python folder
    ...
    _cmd = r"""
cd && rsync -r --exclude machine-learning-with-python_venv --exclude .vagrant --exclude .git --exclude .env /srv/vagrant_repos/machine_learning_with_python/ ~/machine_learning_with_python/ && sudo chown vagrant:vagrant -R ~vagrant && cd ~/machine-learning-with-python && ls -lta
    """
...
pip install -e .
    ...
    ctx.run("machine_learning_with_pythonctl --help")
...
def pstree(ctx, loc="local", verbose=0, cleanup=False):
    ...
    # lsof -nP +c 15 | grep LISTEN
    _cmd = r"""
ps aux | grep 'python machine_learning_with_python/web.py' | awk '{print $2}' | head -1
    """
